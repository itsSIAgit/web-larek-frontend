...
- [Архитектура приложения](#архитектура-приложения)
  - [Базовый код](#базовый-код)
    ...
    - [Класс MasterForm](#класс-masterform)
...
#### Класс Component
Базовый компонент для классов представления.\
Конструктор класса принимает инстант брокера событий и контейнер с разметкой.

Поля класса:
- `events: IEvents` - экземпляр класса EventEmitter для инициации событий при изменении данных
- `container: HTMLElement` - контейнер для разметки

Методы класса:
- описаны в самом классе и предназначены для управления состояниями и полями элементов контейнера
- `render` - принимает объект с данными дженерик типа и сопоставляет их с полями дочернего класса через сеттеры

#### Класс MasterForm
Базовый абстрактный класс для форм ввода данных при оформлении заказа.\
Расширяет Component. По-умолчанию вешает слушателей на событие ввода и отправки формы.

Поля класса:
- `events: IEvents` - экземпляр класса EventEmitter для инициации событий при изменении данных
- `_errors: HTMLElement` - поле для ошибок ввода
- `buttonNext: HTMLButtonElement` - кнопка "Далее" и "Оплатить"
...
Методы класса:
- `setGoods (items: IProduct[], CDN_URL: string): void` - записывает значения массива продуктов и вызывает событие изменения массива каталога
...
- сеттер `buttonLock` - Заблокирует кнопку оплаты, при отправке на сервер, и поменяет ее текст
...
- `big:open` - запрос открытия модального окна с подробным описанием товара
- `card:delete` - нажатие кнопки удаления товара из корзины
- `basket:next`, `order:next`, `contacts:next` - нажатие кнопок "Оформить", "Далее", "Оплатить" в формах
- `payment:click`- нажатие кнопок "Онлайн" и "При получении"
- `${field}:input` - изменение полей ввода address, email, phone (каждый символ)
- `success:next` - нажатие кнопки "За новыми покупками!"
